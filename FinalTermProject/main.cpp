...
//create a template that saves an invitation to a text file
void invitationCard(string partyName, string date, string venueLocation, string time, string venue)
{
    ...
    while(control == 0)
    {
        ...
        ofstream f;
        //text file called Invitation
        f.open("Invitation.txt", ofstream::out | ofstream::trunc);

        cout << endl;
        //outputs first option with partyName, date, venue, and venueLocation
        if(n==1)
        {
            f << "▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒" << endl;
            f << "▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒" << endl;
...
//outputs second option with partyName, date, venue, and venueLocation
        else if(n==2)
        {
...
//outputs third option with partyName, date, venue, and venueLocation
        else if(n==3)
        {
...
//outputs fourth option with partyName, date, venue, and venueLocation
        else if(n==4)
        {
...
//outputs fifth option with partyName, date, venue, and venueLocation
        else if(n==5)
        {
...
//Method to get the price of the food
double Food::getPrice()
{
	return this->price;
}
//Method to get the type of the food
string Food::getFoodType()
{
	return this->foodType;
}
//Method that sets the price of the food
void Food::setPrice(double aPrice)
{
	this->price = aPrice;
}
//Method that sets the type of the food
...
{//returns currentBudget for a Menu object
	return this->currentBudget;
}

Food Menu::getChosenAppetizer()
{//returns the chosenAppetizer for a Menu object
	return this->chosenAppetizer;
}

Food Menu::getChosenEntree()
{//returns the chosenEntree for a Menu object
	return this->chosenEntree;
}

Food Menu::getChosenDessert()
{//returns the chosenDessert for a Menu object
	return this->chosenDessert;
}

Food Menu::getChosenSide()
{//returns the chosenSide for a Menu object
...
{//sets a custom menu item to replace a pre-set menu item
...
{//set a Menu object to all f the pre-set items that a customer can choose from. This allocates the memory for the food objects in the menu class.
	//Appetizers
...
void Menu::printMenu()
{//Prints out the menu by iterating through the menu and outlining the menus categories
	std::cout << "\n\n================================================================================" << endl;
	std::cout << "\tMENU\n===============================================================================\n\n";
	...
	{
		if (menuItems[i].getFoodType() == "Entree")
		{
			entrees++;
		}

		else if (menuItems[i].getFoodType() == "Appetizer")
		{
			appetizers++;
		}

		else if (menuItems[i].getFoodType() == "Dessert")
		{
			desserts++;
		}

		else if (menuItems[i].getFoodType() == "Side")
		{
			sides++;
		}
    ...
    //outline the the menu and divide the categories with lines
	std::cout << "-------------------------------------------------\n";
	cout<< "Appetizers:\n" << endl;
	...
	chosenAppetizer = menuItems[numAppetizer - 1];
	chosenEntree = menuItems[numEntree + appetizers - 1];
	chosenDessert = menuItems[entrees + appetizers + numDessert - 1];
	chosenSide = menuItems[entrees + appetizers + desserts + numSide - 1];

	std::cout<< "\n\n--- All data successfully entered! --- " << endl ;
	std::cout<< "You requested:\n" << endl;

	std::cout<< "\nAppetizer: " << (menuItems[numAppetizer - 1]).getName() << endl;
	std::cout<< "\nEntree: " << (menuItems[numEntree + appetizers - 1].getName()) << endl;
	std::cout<< "\nDessert: " << (menuItems[entrees + appetizers + numDessert - 1].getName()) << endl;
	std::cout<< "\nSide: " << (menuItems[entrees + appetizers + desserts + numSide - 1].getName()) << endl;
...
{//set a person object's name
    this->name = name;
}

std::string Person::getName()
{// return a person objects main
    return name;
}
...
{//construct a guest person object
    this->age = age;
    this->name = name;
}

void Guest::RSVP(bool &coming)
{// set if the guest is coming to the party
    this->isGoing = coming;
}
...
{//delete a person object from the person list
    ...
    if(control != 0)
    {//message if the person has been successfully removed
...
{//get the size of the personList
...
{//set the job title of the staff object
    this->jobTitle = jobTitle;
}

void Staff::setHireCost(double hireCost)
{//set the wages of the staff object
    this->hireCost = hireCost;
}
...
{//get the wages of the staff
    return hireCost;
}

//declaration of checkBudget method to be used in main()
...
{//return the vector of the furniture in inventory object
    return inventory;
}
...
{//add furniture to the inventory class
    int control = 0;

    if(inventory.empty() == false)
    {
        for(unsigned int i = 0; i < inventory.size(); i++)
        {//if the item already exists don't add to inventory
...
{//remove an inputted item of furniture from the Inventory object
...
{//print out all of the relevant furniture information
    ...
    {//if the inventory is empty print out empty
...
{//print out the names of all the furniture items in the inventory object
    ...
    {//if empty print out empty message
...
void Inventory::modify(unsigned int &choice)
{
...
int main()
{
    ...
    while(control == true)
    {
        ...
        cout << "Please enter your budget for " << partyName << ": " << "(A budget of about $10000 is recommended)" << endl;
        cin >> budget;
        ...
        cout << "How many guests will be attending the party? Please enter a \".txt\" file "
             << "with the first name and age of the guests: " << endl;
        string s = " ";
        moneyLeft = budget;
        string filename = " ";
        ...
        if (file.is_open())
        {
            ...
            totalPriceOfStaff = hourlyPriceOfStaff * hoursOfParty;
    ...
    while (!partyIsSet)
    {
        ...
        if (mainLoopChoice == 1)
        {
            ...
            printBudgetBreakdown(partyName, totalPriceOfStaff, budget, moneyLeft, tempMoneySpent, venuePrice, entertainmentPrice, furniturePrice, foodPrice, locationIsSelected, entertainmentIsSelected, furnitureIsSelected, foodIsSelected);
        ...
        else if (mainLoopChoice == 3)
        {
            ...
            while(c == true)
            {
                ...
                cout << "What would you like to add to your inventory? (Provided by given venue, free of charge)" << endl;
                cout << endl;
                cout << "1. Dining Table (for 8 people)" << endl;
                cout << "2. Chair" << endl;
                cout << "3. Drapes" << endl;
                cout << "4. Lamp" << endl;
                ...
                // If the inventory menu option is two, add chairs for each guest
                else if(d=="2")
                {
                ...
                //If the inventory menu option is three, add drapes
                else if(d=="3")
                {
                ...
                //If the inventory menu option is four, add lamps
                else if(d == "4")
                {
                ...
                //If the inventory menu option is five, add chandeliers
                else if(d=="5")
                {
                ...
                //If the inventory menu option is six, add luxury couches
                else if(d=="6")
                {
                ...
                //If the inventory menu option is seven, add none of these furnitures
                else if(d=="7")
                {
                ...
                //If the inventory menu option is eight, finish adding and exit
                else if(d=="8")
                {
                ...
                //If the inventory menu option is invalid, try again
                else
                {
            ...
            while(control == true)
            {
                ...
                //If the furniture menu option is one, add furniture of user's choice
                if(n == "1")
                {
                ...
                //If the furniture menu option is two, remove furniture of the user's choice
                else if(n == "2")
                {
                ...
                //If the furniture menu option is three, print furniture details
                else if(n == "3")
                {
                ...
                //If the furniture menu option is four, modify furniture details
                else if(n == "4")
                {
                ...
                //If the furniture menu option is five, print the total cost of furniture items
                else if(n == "5")
                {
                ...
                //If the furniture menu option is six, exit furniture menu
                else if (n == "6")
                {
                    moneyLeft -= cost;
                ...
                //If the furniture menu option is invalid, try again
                else
                {
            ...
            //Print the current budget breakdown
            printBudgetBreakdown(partyName, totalPriceOfStaff, budget, moneyLeft, tempMoneySpent,  venuePrice, entertainmentPrice, furniturePrice, foodPrice, locationIsSelected, entertainmentIsSelected, furnitureIsSelected, foodIsSelected);
        }

        //If the main loop choice is four, enter food menu
        else if (mainLoopChoice == 4)
        {
            ...
            if (editFood == 1 || !foodIsSelected)
            {
                ...
                //Prompt user to choose the food and print out the menu items
                myMenu->printMenu();
        ...
        //If the main loop choice is incorrect, print incorrect option message
        else
        {
        ...
        //If furniture, entertainment, location, and food is selected, the party planning has finished
        //Output invitation card and itinerary file
        ...
        //If the file name does not exist, print file not opened message
        /*else
        {
...
/*This method takes in the money spent, budget, and money left and
will return false if the budget is being overused and true if under budget.
This method will help the user keep track of spending.
*/
bool checkBudget(double tempMoneySpent, double budget, double moneyLeft)
{
...
/*This method will print out all of the necessary information for the party to an itinerary file.
It will start with a header with the party name and times, then a location.
It will then print the staff and the food options as well as the entertainment.
*/
void makeItineraryFile(string partyName, string partyTime, string partyDate, int hoursOfParty, string partyLocation, int numOfGuests,
                       int numOfWaiters, int numOfBartenders, Menu* myMenu, string entertainmentChoice, Inventory* inventory,
                       double totalPriceOfStaff, double venuePrice, double entertainmentPrice, double furniturePrice, double foodPrice)
{
    ...
    outputFile << endl;
    outputFile << "Length of Party: " << hoursOfParty << " hours" << endl;
    outputFile << endl;
    ...
    for (unsigned int i = 0; i < rInvent.size(); i++)
    {
    ...
    //Print the prices of each category
    ...
    //Use the check budget method to print out the breakdown of the event
